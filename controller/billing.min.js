...
router.post("/",auth,(a,d)=>{stripe.charges.create({amount:500,currency:"usd",description:"Subscription for 5 Email Credits",source:a.body.id},(b,f)=>{b&&console.log(b);User.findById(a.user._id).then(c=>{c.credit+=5;c.save();then(e=>{d.status(200).json({data:e})})})})});module.exports=router;